...
export const load = ({ url }) => {
  return {
    ...defineBaseMetaTags({
      ...
      canonical: new URL(url.pathname, url.origin).href, // 現在のURLからクリーンなURL（ハッシュやクエリパラメータなし）を作成
      openGraph: {
        type: 'website',
...
> 注意: `defineBaseMetaTags`は、その出力を`+layout.(j|t)s`ファイルにスプレッドして使用することを意図したユーティリティです。
> これにより、フリーズされたオブジェクトを含む`baseMetaTags`プロパティが追加されます。
> また、`props`プロパティを通してフリーズされたオブジェクトを使用することもできます。
>
...
> 注意: `defineBaseMetaTags`と同様に、`definePageMetaTags`は、その出力を`+page.(j|t)s`ファイルにスプレッドして使用することを意図したユーティリティです。
> これにより、フリーズされたオブジェクトを含む`pageMetaTags`プロパティが追加されます。
> また、`props`プロパティを通してフリーズされたオブジェクトを使用することもできます。
>
