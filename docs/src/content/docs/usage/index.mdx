...
> Note: Using `await` expressions requires enabling `experimental.async` in the Svelte compiler options. When using `await` in the markup, you must wrap it with `<svelte:boundary>`. Using `$derived` ensures the query re-runs on client-side navigation when parameters change.
